...
async def index_files(bot, query):
    if query.data.startswith('index_cancel'):
        temp.CANCEL = True
        return await query.answer("Wɪ8 A Mɪɴᴜᴛᴇ Sᴛᴏᴘɪɴɢ Tʜᴇ Iɴᴅᴇx Mᴇᴀɴs Sᴏ Mᴀɴʏ Hᴀᴄᴋɪɴɢs")
    _, raju, chat, lst_msg_id, from_user = query.data.split("#")
    if raju == 'reject':
        await query.message.delete()
        await bot.send_message(int(from_user),
                               f'Your Submission for indexing {chat} has been decliened by our moderators Dᴇᴄʟɪɴᴇᴅ ʏᴏᴜʀ ᴄʜᴀɴɴᴇʟ.',
    ...
    await query.answer('Hᴀᴄᴋɪɴɢ Tʜᴇ Dᴀᴛᴀʙᴀsᴇ', show_alert=True)
    if int(from_user) not in ADMINS:
        await bot.send_message(int(from_user),
                               f'Your Submission for indexing {chat} has been accepted by our moderators and will be added soon.',
                               reply_to_message_id=int(lst_msg_id))
    await msg.edit(
        "Sᴛᴀʀᴛᴇᴅ Hᴀᴄᴋɪɴɢ Sᴛᴇᴀʟɪɴɢ Fɪʟᴇs ᴀsᴀᴘ",
